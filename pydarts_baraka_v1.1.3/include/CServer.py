...
from include.config import pyDartsVersion
...
#
   ...
   def SendVersion(self,conn,gamename,addr):
      d={'GAMENAME':gamename,'REQUEST':'VERSION','VERSION':pyDartsVersion}
      self.SendMessage2(conn,gamename,addr,d)
...
#
   ...
   def Play(self,gamename,hit,actualround,actualplayer,playerlaunch):
      for LstGames in self.Games:
         if LstGames.GameName == gamename:
            """ OLD FASIONED WAY
            # Case of BACKUPBUTTON
            #debug-print("Hit {} actualround {} actualplayer {} playerlaunch {}".format(hit,actualround,actualplayer,playerlaunch))
            if hit=='BACKUPBUTTON' and playerlaunch>1 and playerlaunch<=LstGames.nbdarts: # If the player has played some darts, we go back to dart 1
               self.Logs.Log("WARNING","BackupButton during a launch. Deleting first dart of this player. Case 1")
               nexthit="{}{}{}".format(actualround,actualplayer,1)
            elif hit=='BACKUPBUTTON' and playerlaunch==1 and actualplayer>0: # If the player has played no dart yet, we go back to previous player
               self.Logs.Log("WARNING","BackupButton at first launch. Deleting first dart of previous player. Case 2")
               nexthit="{}{}{}".format(actualround,actualplayer-1,1)
            elif hit=='BACKUPBUTTON' and playerlaunch==1 and actualplayer==0: # If the player has played no dart yet and is player 1, we turn back to the previous round
               self.Logs.Log("WARNING","BackupButton for first player. Deleting first dart of previous player, in previous round. Case 3")
               nexthit="{}{}{}".format(actualround-1,len(LstGames.PlayersNames)-1,1)
            # Case of PLAYERBUTTON
            elif hit=='PLAYERBUTTON' and actualplayer==len(LstGames.PlayersNames)-1: # Deleting next dart in case of playerbutton for last player
               self.Logs.Log("WARNING","PLAYERBUTTON for last player ! Deleting first dart of first player, in next round. Case 4")
               nexthit="{}{}{}".format(actualround+1,0,1)
            elif hit=='PLAYERBUTTON' and actualplayer<len(LstGames.PlayersNames)-1: # Deleting next dart in case of playerbutton for other players
               self.Logs.Log("WARNING","PLAYERBUTTON ! Deleting first dart of next player, in this round. Case 5")
               nexthit="{}{}{}".format(actualround,actualplayer+1,1)
            # Else we delete data stored for next dart (FORWARD - usual case)
            elif playerlaunch<LstGames.nbdarts: # If player is not the last of the round
               nexthit="{}{}{}".format(actualround,actualplayer,playerlaunch+1)
            elif playerlaunch==LstGames.nbdarts and actualplayer<len(LstGames.PlayersNames)-1: # Player has played all his darts
               nexthit="{}{}{}".format(actualround,actualplayer+1,1)
            elif playerlaunch==LstGames.nbdarts and actualplayer==len(LstGames.PlayersNames)-1: # Last dart of last player of the round
               nexthit="{}{}{}".format(actualround+1,0,1)
            try:
               oldvalue = LstGames.HitValues[nexthit]
               del LstGames.HitValues[nexthit] # Try to del Hit stored as previous (before backup)
               del LstGames.RandomValues[nexthit] # Try to delete stored random values (before backup)
               self.Logs.Log("WARNING","Deleted data ({}) for launch {} for game {} (next round)".format(oldvalue,nexthit,gamename))
            except Exception as e: # In case of problem
               self.Logs.Log("WARNING","Unable to delete next hit & random for {}".format(nexthit)) # Nothing
            # END OF OLD FASHIONED WAY
            """
